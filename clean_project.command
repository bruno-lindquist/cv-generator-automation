...
echo "Preservando ambientes virtuais: .venv e venv"
echo
...
# Remove __pycache__ e *.egg-info fora de .venv/venv/.git.
find . \
  \( -path "./.venv" -o -path "./venv" -o -path "./.git" \) -prune \
...
# Remove arquivos temporários fora de .venv/venv/.git.
find . \
  \( -path "./.venv" -o -path "./venv" -o -path "./.git" \) -prune \
  -o -type f \
